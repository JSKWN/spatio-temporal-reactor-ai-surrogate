...
```
면 중성자류 (방법론 Eq. 2.1-37):
  J = D̃ × (φ_이웃 - φ_중심)
  D̃ = 2·D_m·D_{m±1} / (a_{m±1}·D_m + a_m·D_{m±1})
  균일 메시: D̃ = D_harmonic / h  (조화평균 ÷ 노드간 거리)

  조화평균: D_harm = 2·D_a·D_b / (D_a + D_b)
...
$$\mathcal{L}_{diffusion} = \frac{1}{B \cdot N_{inner}} \sum_{b,v} (R_{g1}^2 + R_{g2}^2)$$

- B = batch size, N_inner = inner 연료 노드 수 (경계 노드 제외)
- Inner-only 평가: 경계 조건(albedo 등) 처리 불필요

### 3.4 사전 검증 결과 (2026-03-30)
...
- g1 median 3.50% → MARGINAL
- g2 median 6.99% → ACCEPTABLE (CMFD vs NNEM 구조적 차이, NEM D̂ 미포함)
...
| 단계 | λ_data | λ_Bateman | λ_σXe | λ_diffusion | λ_keff |
|------|:-----:|:---------:|:--------:|:-----------:|:------:|
